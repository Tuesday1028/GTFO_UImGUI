...
namespace Hikaria.UImGUI.Platform
{
	...
	internal class PlatformBase : IPlatform
	{
		...
		public virtual bool Initialize(ImGuiIOPtr io, ImGuiPlatformIOPtr platformio, UIOConfig config, string platformName)
		{
			...
			_callbacks.Assign(io, platformio);
		...
		public virtual void PrepareFrame(ImGuiIOPtr io, Rect displayRect)
		{
			if (!io.Fonts.IsBuilt())
			{
				Logs.LogError("Font atlas not built! Generally built by the renderer. Missing call to renderer NewFrame() function?");
				return;
			}
