...
namespace ImGuiNET
{
    ...
    sealed class ImGuiRendererProcedural : IImGuiRenderer
    {
        ...
        unsafe void CreateOrResizeIdxBuffer(ref GraphicsBuffer buffer, int count)
        {
            int num = ((count - 1) / 256 + 1) * 256;
            buffer?.Release();
            buffer = new GraphicsBuffer(UnityEngine.GraphicsBuffer.Target.Index, num, sizeof(uint));
        }
        ...
        unsafe void UpdateBuffers(ImDrawDataPtr drawData)
        {
            ...
            for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
            {
                ...
                uint[] idxArray = new uint[drawList.IdxBuffer.Size];
                fixed (uint* pDest = idxArray)
                {
                    Buffer.MemoryCopy(
                         (void*)drawList.IdxBuffer.Data,
                         pDest,
                         idxArray.Length * sizeof(uint),
                         idxArray.Length * sizeof(uint)
                     );
                    _idxBuf.InternalSetNativeData((IntPtr)pDest, 0, idxOf, idxArray.Length, sizeof(uint));
                }
