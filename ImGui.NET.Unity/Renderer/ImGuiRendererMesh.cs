    ...
    sealed class ImGuiRendererMesh : IImGuiRenderer
    {
        ...
        unsafe void UpdateMesh(ImDrawDataPtr drawData, Vector2 fbSize)
        {
            ...
            for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
            {
                ...
                _mesh.InternalSetVertexBufferDataFromArray(vtxArray, 0, vtxOf, vtxArray.Length, 0, NoMeshChecks);
                _mesh.InternalSetIndexBufferDataFromArray(idxArray, 0, idxOf, idxArray.Length,    NoMeshChecks);
            ...
            //_mesh.SetSubM(descriptors, NoMeshChecks);
